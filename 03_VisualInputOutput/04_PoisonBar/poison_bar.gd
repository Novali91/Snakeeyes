...
func _process(delta: float) -> void:
	...
	if _change_progress >= 0.0 and _change_progress <= 1.0:
		...
		mat.set_shader_parameter("level",lerp_eased(poison_heights[clamp(_old_level-1,0,12)],poison_heights[clamp(_new_level-1,0,12)],_change_progress))
	else:
		_change_progress = -1.0
		mat.set_shader_parameter("level",poison_heights[clamp(_new_level-1,0,12)])
